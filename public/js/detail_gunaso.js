...
async function loadGunaso(){
    try {
        ...
        const res = await fetch(`/api/gunaso/${id}`);
        if(!res.ok) throw new Error("Network response was not ok");
        const data = await res.json();
        const container = document.getElementById("datas");
        container.innerHTML = "";
        const gunasoCard = createGunasoCard(data);
        container.appendChild(gunasoCard);
        attachLikeHandler(data._id);
        attachShareHandler(data._id);
        attachCommentHandler(data._id);
...
function attachLikeHandler(gunasoId){
    const likeBtn = document.querySelector(".like-btn-1");
    const likeCountSpan = document.getElementById(`like-count-${gunasoId}`);
    if(!likeBtn) return;
...
function attachCommentHandler(gunasoId){
    const commentForm = document.querySelector(".comment-input-section");
    if(!commentForm) return;
    commentForm.addEventListener("submit", async(event)=> {
        event.preventDefault();
        const commentText = document.getElementById("comment-input").value;
        try {
            const res = await fetch(`/api/gunaso/${gunasoId}/comment`, {
                method: "POST",
                headers: {
    ...
    try {
        ...
        comments.forEach(element => {
            ...
            `
                            ...
                            <span>${element.userId?.display_name || "Anonymous User"} </span>
                            <span> ${timeAgo(element.createdAt)} </span>
                        </div>
